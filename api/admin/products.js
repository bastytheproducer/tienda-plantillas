...
module.exports = async (req, res) => {
  ...
  if (req.method === "POST") {
    ...
    if (!product?.id || !product?.name || !product?.price || !product?.style || !product?.volume) {
      return res.status(400).json({ error: "Faltan campos: id, name, style, volume y price son obligatorios" });
    }
