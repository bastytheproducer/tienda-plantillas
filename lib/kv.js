...
const KV_KEY = "barrilymiel:products:v2";
...
async function getProducts() {
  ...
  remote.forEach((p) => {
    if (!p || !p.id) return;
    if (typeof p.price === "number") priceById[p.id] = p.price;
  });

  const merged = local.map((p) => {
    if (typeof priceById[p.id] === "number") {
      return { ...p, price: priceById[p.id] };
    }
    return p;
  });

  // Conserva productos nuevos que se hayan creado desde el panel admin
  // y que todavía no estén en el repositorio.
  const extras = remote.filter((p) => p && p.id && !localIds.has(p.id));
